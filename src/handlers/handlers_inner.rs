...
use super::{
    persistence::{
        answers_dao::{self, AnswersDao},
        questions_dao::{self, QuestionsDao},
    },
    Answer, AnswerDetail, AnswerId, DBError, Question, QuestionDetail, QuestionId,
};
...
pub async fn create_question(
    ...
    match questions_dao.create_question(question).await {
...
pub async fn delete_question(
    QuestionId { question_uuid }: QuestionId,
    questions_dao: &(dyn QuestionsDao + Send + Sync),
) -> Result<(), HandlerError> {
    if let Err(_) = questions_dao.delete_question(question_uuid).await {
...
pub async fn create_answer(
    ...
    match answers_dao.create_answer(answer).await {
        Ok(answer) => Ok(answer),
        Err(err) => match err {
            DBError::InvalidUUID(err_string) => Err(HandlerError::BadRequest(format!(
                "Error passing in s: `{}`  as the string",
                err_string
            ))),
...
pub async fn read_answers(
    ...
    match answers_dao.get_answers(question_uuid).await {
...
pub async fn delete_answer(
    AnswerId { answer_uuid }: AnswerId,
    answers_dao: &(dyn AnswersDao + Send + Sync),
) -> Result<(), HandlerError> {
    if let Err(err) = answers_dao.delete_answer(answer_uuid).await {
        return Err(HandlerError::default_internal_error());
    }
